    ...
    it('renders without crashing', () => {
        render(
            <DetailsModel
                showModel={true}
                data={data}
                unique={data.created.replace(/\D/g, '')}
                setShowModel={() => { }}
            />
        );
    });

    it('displays correct character information in the modal', () => {
        render(
            <DetailsModel
                showModel={true}
                data={data}
        ...
        // Check if the modal displays the character's name
        expect(screen.getByText(data.name)).toBeInTheDocument();

        const heightValue = data.height / 100;
        let regexPattern = new RegExp(heightValue, 'i');
        expect(screen.getByText(regexPattern)).toBeInTheDocument();

        // Check if the modal displays the character's mass
        const massValue = data.mass;
        regexPattern = new RegExp(massValue, 'i');
        expect(screen.getByText(regexPattern)).toBeInTheDocument();

        // Check if the modal displays the number of films the character appeared in
        const films = '^' + data.films.length + '$';
        regexPattern = new RegExp(films, 'i');
        expect(screen.getByText(regexPattern)).toBeInTheDocument();

        // Check if the modal displays the character's birth year
        const byear = data.birth_year;
        regexPattern = new RegExp(byear, 'i');
        expect(screen.getByText(regexPattern)).toBeInTheDocument();

        // Check if the modal displays the character's created date
        const date = '^15-12-2014$';
        regexPattern = new RegExp(date, 'i');
        expect(screen.getByText(regexPattern)).toBeInTheDocument(); // Mock date is "2022-05-04"

        // Check if the modal displays the character's homeworld information
        const pname = data.homeworlddata.name;
        regexPattern = new RegExp(pname, 'i');
        expect(screen.getByText(regexPattern)).toBeInTheDocument();

        const tname = data.homeworlddata.terrain;
        regexPattern = new RegExp(tname, 'i');
        expect(screen.getByText(regexPattern)).toBeInTheDocument();

        const climate = data.homeworlddata.climate;
        regexPattern = new RegExp(climate, 'i');
        expect(screen.getByText(climate)).toBeInTheDocument();

        const popu = data.homeworlddata.population;
        regexPattern = new RegExp(popu, 'i');
        expect(screen.getByText(regexPattern)).toBeInTheDocument();
    });

    it('should close the modal when "Close" button is clicked', () => {
        const setShowModel = jest.fn();
        ...
        // Check if setShowModel function is called with false
        expect(setShowModel).toHaveBeenCalledWith(false);
    });

    it('should navigate to the second modal when "HomeWorld Details" button is clicked', () => {
        const setShowModel = jest.fn();
        ...
        // Find and click the "HomeWorld Details" button
        fireEvent.click(screen.getByText(/HomeWorld Details/i, { selector: '.btn.btn-danger' }));
